...
public class cust_register_Controller {
    ...
    public boolean checkInputs() throws IOException, LineUnavailableException, UnsupportedAudioFileException {
        ...
        if (cnic.getText().matches(".*[a-zA-Z]+.*") || cnic.getText().length() != 13) {
            openPopup("Invalid Input", "CNIC should contain only numbers, dashes, and be exactly 13 characters long.");
            return false;
        }


        // invalid email address
        if (!email.getText().matches("^[\\w.-]+@gmail\\.com$") || email.getText().length()>50 ||
                !pass.getText().matches("^(?=.*[A-Z])(?=.*[!@#$%^&*]).+$")) {
            openPopup("Invalid Input", "Please ensure the email is a valid Gmail address and the password contains at least one uppercase letter and one special character.");
            return false;
        }
