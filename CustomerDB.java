...
public class CustomerDB
{
    ...
    public String getCustomerMessageByID(String custID) {
        String resultMessage = "";  // Default empty result

        // Establish a connection to the database
        try (Connection conn = DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/" + Globals.getDb_name(),
                Globals.getDb_username(),
                Globals.getDb_pass())) {

            // Check if the connection was successful
            if (conn != null) {
                System.out.println("Database connected - returning customer message by ID");

                // Prepare and execute the SQL query
                String query = "SELECT message FROM customer_messages WHERE cust_id = ?";
                try (PreparedStatement pstmt = conn.prepareStatement(query)) {
                    pstmt.setString(1, custID);  // Set the custID in the query
                    ...
                    if (rs.next()) {
                        ...
                        System.out.println("No message found for customer ID: " + custID);
                    }
                }
            } else {
                System.out.println("Failed to make connection!");
